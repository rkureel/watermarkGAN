...
def save_checkpoint(model: WatermarkGAN, experiment_name: str, epoch: int, checkpoint_folder: str):
...
# def load_checkpoint(WatermarkGAN_net: WatermarkGAN, options: Options, this_run_folder: str):
...
def model_from_checkpoint(WatermarkGAN_net, checkpoint):
    """ Restores the WatermarkGAN_net object from a checkpoint object """
    WatermarkGAN_net.encoder_decoder.load_state_dict(checkpoint['enc-dec-model'])
    WatermarkGAN_net.optimizer_enc_dec.load_state_dict(checkpoint['enc-dec-optim'])
    WatermarkGAN_net.discriminator.load_state_dict(checkpoint['discrim-model'])
    WatermarkGAN_net.optimizer_discrim.load_state_dict(checkpoint['discrim-optim'])
...
def load_options(options_file_name):
    ...
    with open(os.path.join(options_file_name), 'rb') as f:
        ...
        WatermarkGAN_config = pickle.load(f)
        # for backward-capability. Some models were trained and saved before .enable_fp16 was added
        if not hasattr(WatermarkGAN_config, 'enable_fp16'):
            setattr(WatermarkGAN_config, 'enable_fp16', False)

    return train_options, WatermarkGAN_config, noise_config


def get_data_loaders(WatermarkGAN_config: WatermarkGANConfiguration, train_options: TrainingOptions):
    ...
    data_transforms = {
        'train': transforms.Compose([
            transforms.RandomCrop((WatermarkGAN_config.H, WatermarkGAN_config.W), pad_if_needed=True),
            transforms.ToTensor(),
            transforms.Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5])
        ]),
        'test': transforms.Compose([
            transforms.CenterCrop((WatermarkGAN_config.H, WatermarkGAN_config.W)),
            transforms.ToTensor(),
            transforms.Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5])
        ])
    }
